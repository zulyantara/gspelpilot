...
class Pelatih_lanjutan_model extends CI_Model
{
    ...
    function getsesi($id){
        ...
        $qry = "SELECT a.nama_intake as nama_intake,a.id as id_intake FROM ref_intake a LEFT JOIN settings_tawaran_kursus b ON a.id=b.id_intake LEFT JOIN ref_kursus c ON b.id_kursus=c.id WHERE c.id ='$id' GROUP BY nama_intake, id_intake";
        // $this->db->group_by('nama_intake');
       //  var_dump($qry);die();
    ...
    * @return
    ...
    //mmn add to print surat temuduga
    ...
    //                    a.id_giatmara = d.id
    //           ";
      $qry = "SELECT * FROM v_cetakan_surat_temuduga_lpp09 WHERE id_temuduga_tetapan=".$id;
      // echo $qry;exit;
...
//$this->db->where('jenis_pelatih', 'LPP09');
        ...
        return $qry->row();
         ...
         /*$qry = "SELECT * FROM v_cetak_tawaran_kursus_lpp04 WHERE jenis_pelatih='LPP09'";
    ...
    * @return
    ...
    function insert_maklumat_penjaga($data) {
        // echo $this->db->set($data)->get_compiled_insert("permohonan_penjaga");exit;
    ...
    * @return
    ...
    {
        $query  ="select * from ref_giatmara where id_negeri='$id' and id_type_giatmara=1 ORDER BY nama_giatmara ASC";
    ...
    function cek_idpermohonan($idpermohonan,$idpermohonankursuslpp09){
        //code ori return $this->db->get_where($table,$where);

        $query = "SELECT * FROM kelulusan_lpp09 WHERE id_permohonan='".$idpermohonan."' AND
                    id_permohonan_kursus_lpp09='".$idpermohonankursuslpp09."'";//echo $query;exit;
                    return $this->db->query($query);
    ...
    function get_data_kelulusan($negeri,$giatmara)
    {
        ...
        $query  ="select * from v_kelulusan_permohonan_lpp09_2 where id_negeri='$negeri' AND id_giatmara='$giatmara' AND (status_pengesahan IS NULL OR status_pengesahan=0)";
        // echo $query;exit;
        return $this->db->query($query);
    }
    ...
    {
        //not use $query  ="SELECT DISTINCT(nama_penuh),no_mykad,no_pelatih,no_hp,kategori_program,tawaran_terus,tarikh_mohon,id_negeri,nama_negeri,id_giatmara,id_permohonan, nama_giatmara, nama_kursus, id_permohonan_kursus_lpp09 FROM v_kelulusan_permohonan_lpp09 WHERE (status_pengesahan IS NULL OR status_pengesahan = 0) OR keputusan_temuduga IN (2,3) ORDER BY id_permohonan_kursus_lpp09 DESC";
        //code asal $query  ="SELECT DISTINCT(nama_penuh),no_mykad,no_pelatih,no_hp,kategori_program,tawaran_terus,tarikh_mohon,id_negeri,nama_negeri,id_giatmara,id_permohonan, nama_giatmara, nama_kursus, id_permohonan_kursus_lpp09 FROM v_kelulusan_permohonan_lpp09 WHERE (status_pengesahan IS NULL OR status_pengesahan = 0) ORDER BY id_permohonan_kursus_lpp09 DESC";
        $query  ="select * from v_kelulusan_permohonan_lpp09_2 where (status_pengesahan IS NULL OR status_pengesahan=0)";
        // echo $query;
        return $this->db->query($query);
    }
    ...
    function get_data_seluruh_4b_pendaftaran()
    {
        //  get info user login
        $user = $this->ion_auth->user()->row();
        $user_staff = $user->id_staff;
        ...
        $query  ="SELECT * FROM v_coba_screen4b_lpp09 WHERE status_pengesahan='1' AND status_pendaftaran IS NULL AND idgiatmara=".$giatmara_id." ORDER BY id_permohonan_kursus_lpp09 DESC";
        return $this->db->query($query);
    }
    ...
    {
        $query  ="SELECT * FROM v_coba_screen4b_lpp09
            WHERE status_pendaftaran IS NULL AND idnegeri='$negeri' AND idgiatmara='$giatmara' AND kategori='$kategori'";
        return $this->db->query($query);
    }
     ...
     */
    function get_settings_tawaran_kursus($where = FALSE)
    {
        if ($where !== FALSE)
        {
            if (is_array($where))
        ...
        $this->db->select("stk.id, stk.id_kursus, stk.id_giatmara, stk.id_intake, k.kod_kursus, k.nama_kursus, i.nama_intake, i.tarikh_mula, i.tarikh_tamat");
        $this->db->join("ref_kursus k", "k.id = stk.id_kursus");
        $this->db->join("ref_giatmara g", "stk.id_giatmara = g.id");
        $this->db->join("ref_intake i", "stk.id_intake = i.id");
        // $this->db->where("status_kursus", 1);
        // echo $this->db->get_compiled_select("settings_tawaran_kursus stk");exit;
        return $this->db->get("settings_tawaran_kursus stk");
    }
    ...
    function get_lpp09_p4a($where = NULL, $return = "result")
    {
        ...
        //ori code $this->db->where('kategori_program', "LL");
        //ori code $this->db->where('tawaran_terus IN (1,0)');
        //ori code $this->db->where('(status_pendaftaran !=5 OR status_pendaftaran IS NULL)');
        // $this->db->or_where('status_pendaftaran IS NULL');
        //ori code $this->db->where('(CASE WHEN tawaran_terus = 1 THEN (CASE WHEN status_pengesahan = 1 THEN TRUE ELSE FALSE END) ELSE TRUE END)');
    ...
    {
        $this->load->model('Programreg_model');

        $this->db->trans_start();

        // echo "<pre>";var_dump($data);echo "</pre>";exit;
        $id_kelulusan_lpp09 = $data["id_kelulusan_lpp09"];
        $id_permohonan = $data["id_permohonan"];
        $id_kursus = $data["id_kursus"];
        $id_stk = $data["id_stk"];
        $id_temuduga_tetapan = $data["id_temuduga_tetapan"];
        $no_pelatih = $this->Programreg_model->generate_no_pelatih();
        ...
        $tarikh_lapor = $data["txt_tarikh_lapor"];
        $masa_lapor = $data["opt_masa_lapor"];
        $masa_menit_lapor = $data["opt_masa_menit_lapor"];
        $status = $data["opt_sah"];
        ...
        foreach ($chk_tindakan as $key => $val)
        {
            ...
            $datakl["status_pendaftaran"] = $status[$val];
            $this->db->where('id', $id_kelulusan_lpp09[$val]);
            // echo $this->db->set($datakl)->get_compiled_update("kelulusan_lpp09")."<br>";
            $returnx = $this->db->update('kelulusan_lpp09', $datakl);
            #$error  = $this->db->error ();
            #if ( $error['code'] === 0  &&  $returnx === true ) {
            #    echo "1=Success!<pre>";print_r($returnx);echo "</pre>";
            #    echo "affected ".$this->db->affected_rows()."<br/>";
            #    echo "1=".$this->db->last_query()."<br/>";
            #} else {
            #    echo "1=Gagal!<pre>";print_r($error);echo "</pre>";
            #}

            $datatt["pendaftaran_status"] = $status[$val];
            $this->db->where('id_permohonan', $id_permohonan[$val]);
            $this->db->where('jenis_pelatih', "LPP09");
            // echo $this->db->set($datatt)->get_compiled_update("temuduga_tetapan")."<br>";
            $returnx = $this->db->update('temuduga_tetapan', $datatt);
            #$error  = $this->db->error ();
            #if ( $error['code'] === 0  &&  $returnx === true ) {
            #    echo "2=Success!<pre>";print_r($returnx);echo "</pre>";
            #    echo "affected ".$this->db->affected_rows()."<br/>";
            #    echo "2=".$this->db->last_query()."<br/>";
            #} else {
            #    echo "2=Gagal!<pre>";print_r($error);echo "</pre>";
            #}

            /*echo "<br>".$qry = "UPDATE kelulusan_lpp09 SET status_pendaftaran='".$status[$val]."' WHERE id=".$id_kelulusan_lpp09[$val];
            $data = $this->db->query($qry);

            echo "<br>".$qry2 = "UPDATE temuduga_tetapan SET pendaftaran_status='".$status[$val]."' WHERE id_permohonan=".$id_permohonan[$val]." AND jenis_pelatih='LPP09'";
            $data = $this->db->query($qry2);*/

            $upd_pelatih['id_permohonan'] = $val;
            $upd_pelatih['no_pelatih'] = $no_pelatih[$val];//code asal$this->generate_no_pelatih();
            ...
            $upd_pelatih['kelayakan_elaun'] = ($kelayakan_elaun[$val]!="")?$kelayakan_elaun[$val]:0;
            ...
            $upd_pelatih["tarikh_lapordiri"] = date("Y-m-d", strtotime($tarikh_lapor));
            $upd_pelatih["masa_lapordiri"] = $masa_lapor.":".$masa_menit_lapor;
            // echo $this->db->set($upd_pelatih)->get_compiled_insert("pelatih");
            $returnx = $this->db->insert('pelatih', $upd_pelatih);
            $id_insert_pelatih = $this->db->insert_id();
            #$error  = $this->db->error ();
            #if ( $error['code'] === 0  &&  $returnx === true ) {
            #    $id_insert_pelatih = $this->db->insert_id();
            #    echo "3=Success!<pre>";print_r($returnx);echo "</pre>";
            #    echo "last id insert pelatihh=".$id_insert_pelatih."<br/>";
            #    echo "affected ".$this->db->affected_rows()."<br/>";
            #    echo "3=".$this->db->last_query()."<br/>";
            #} else {
            #    echo "3=Gagal!<pre>";print_r($error);echo "</pre>";
            #    echo "3=".$this->db->last_query()."<br/>";
            #}
            ...
            if (count($rs_m1))
            {
                foreach ($rs_m1 as $m1)
                {
                    ...
                    #$qqq = $this->db->insert_id();
                    #echo "insert kursus ".$qqq."<br/>";
                }
                // exit;
            }
            ...
            if ($layak_elaun[$val]  != 1)
            {
                ...
                #echo $this->db->set($datalpp09)->get_compiled_insert("lpp_09")."<br/>";
                $this->db->insert('lpp_09', $datalpp09);
            }
           # $this->db->db_debug = $db_debug           ; //set it back
        }
        $this->db->trans_complete();
        // $trans_status = $this->db->trans_status();exit;
        #if (!$trans_status) echo "Gagal transaksi ".$trans_status."..!<br/>";
        #else  echo "Ok transaksi ".$trans_status."..!<br/>";
    }
    ...
    //mmn add
    function get_cetak_tawaran_kursus_lpp09($where)
    {
        foreach ($where as $key => $val)
        {
            $this->db->where($key, $val);
        ...
        $this->db->distinct();
        $qry = $this->db->get('v_cetak_surat_tawaran_lpp09');
        return $qry->row();
    }

    function get_last_nomor_surat_temuduga($where = NULL)
    {
        if ($where !== NULL)
        {
            foreach ($where as $key => $value)
        ...
        $this->db->select("nomor_surat_temuduga");
        $this->db->order_by("nomor_surat_temuduga", "desc");
        // echo $this->db->get_compiled_select("temuduga_tetapan");exit;
        return $this->db->get("temuduga_tetapan");
    }

    function get_last_nomor_surat_tawaran($where = NULL)
    {
        if ($where !== NULL)
        {
            foreach ($where as $key => $value)
        ...
        $this->db->select("nomor_surat_tawaran");
        $this->db->order_by("nomor_surat_tawaran", "desc");
        // echo $this->db->get_compiled_select("temuduga_tetapan");exit;
        return $this->db->get("temuduga_tetapan");
    }

    function get_senarai_permohonan_lpp09($where = FALSE)
    {
      if ($where !== FALSE)
      {
        foreach ($where as $key => $value)
      ...
      // echo $this->db->get_compiled_select("v_senarai_permohonan_lpp09");exit;
      return $this->db->get("v_senarai_permohonan_lpp09");
    }
}

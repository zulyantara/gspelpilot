...
//                    $html .= '$(".modal-body #kekerapan").val("' . $kewPotongan->deduction_frequency . '");';
                    $html .= '$(".modal-body #kekerapan").val("1");';
                ...
                echo $html;
            } elseif ($task == "get_jumlah_potongan") {
                $id = $this->input->post('id') ? $this->input->post('id') : 0;
                $jumlah = $this->input->post('jumlah') ? $this->input->post('jumlah') : 0;

                $this->load->model('V_kew_potongan_model');
                $kewPotongan = $this->V_kew_potongan_model->get_by_id($id);

                if ($kewPotongan) {
                    $html = '$(".modal-body #jumlah").val("' . $jumlah * $kewPotongan->value_per_unit . '");';
                } else {
                    $html = '$(".modal-body #jumlah").val("");';
                }
                echo $html;
...
//        print_r(urldecode($this->uri->segment(4)));exit;
        if ((!empty($this->input->post())) || (!empty(urldecode($this->uri->segment(5))))) {
            if (!empty($this->input->post())) {

                if ($_POST['action'] == 'papar') {
                    $this->mViewData['idNegeri'] = NULL; //$this->input->post('negeri');
                    $this->mViewData['idGiatmara'] = $giatmara_id; //$this->input->post('giatmara');
                ...
                $this->mViewData['idPelatih'] = urldecode($this->uri->segment(4));
                $this->mViewData['idNegeri'] = NULL; //urldecode($this->uri->segment(5));
                $this->mViewData['idGiatmara'] = $giatmara_id; //urldecode($this->uri->segment(6));
                ...
                $this->mViewData['dataLpp7a'] = $this->Lpp_7a_model->get_by_id_pelatih($this->mViewData['idPelatih'], "dihantar_oleh is null");
//                print_r($this->mViewData['dataLpp7a']);exit;
                foreach ($this->mViewData['dataLpp7a'] as $key => $value) {
                    $idUser = $this->ion_auth->user()->row()->id;
                    $data = array(
...
//                    print_r($value);
                    ...
                    $this->Lpp_7a_model->update($value->id, $data);
                }
//                exit;
            }
...
//            echo $this->mViewData['namaNegeri'];
//            $this->mViewData['namaGiatmara'] = $this->Ref_giatmara_model->get_by_id($this->mViewData['idGiatmara'])->nama_giatmara;
            ...
            $this->mViewData['data'] = $this->V_kursus_terdahulu_model->get_all($this->mViewData['idGiatmara'], $this->mViewData['idKursus'], NULL, $this->mViewData['idIntake'], $this->mViewData['idNegeri'], $this->mViewData['nama'], "id_status_pelatih = 1");
...
//            exit;
    ...
    public function batal_pelatih() {
        ...
        if (!empty($this->input->post())) {
            ...
            $this->mViewData['dataPelatih'] = $this->V_kursus_terdahulu_model->get_all($giatmara_id, $this->mViewData['idKursus'], $this->mViewData['katPelatih'], $this->mViewData['idIntake'], NULL, NULL, "(id_pelatih not in (select a.id_pelatih from lpp_08 a where a.status_jana = 1)) and id_status_pelatih = 1");
        }
    ...
    public function tukar_giatmara_kursus() {
        ...
        if (!empty($this->input->post())) {
            ...
            $this->mViewData['dataPelatih'] = $this->V_kursus_terdahulu_model->get_all($giatmara_id, $this->mViewData['idKursus'], $this->mViewData['katPelatih'], $this->mViewData['idIntake'], NULL, NULL, "id_status_pelatih = 1 and id_markah is null");
        }
...
//                    'cara_bayaran_baru' => $this->input->post('cara_bayaran_baru'),
            ...
            $this->mViewData['dataPelatih'] = $this->V_kursus_terdahulu_model->get_all($giatmara_id, $this->mViewData['idKursus'], $this->mViewData['katPelatih'], $this->mViewData['idIntake'], NULL, NULL, "id_status_pelatih = 1");
        }
        $this->mPageTitle = "Pengurusan Pelatih";
...
//            print_r($_POST['action']); exit;
            if ($_POST['action'] == 'hantar') {
                ...
                $data = array(
                    ...
                    'kekerapan' => $this->input->post('kekerapan'),
...
//                    'dihantar_pada' => date('Y-m-d H:i:s'),
                ...
                if (isset($_POST['pilih'])) {
//                    print_r($_POST['pilih']); exit;
                    $this->load->model('Lpp_7a_model');
                    foreach ($_POST['pilih'] as $key => $value) {
                        $this->Lpp_7a_model->delete($key);
                    }
                }
...
//                $this->load->model('Lpp_7a_model');
//                $dt = $this->Lpp_7a_model->get_by_id_pelatih($idPelatih, "dihantar_oleh is null");
////                print_r($this->mViewData['dataLpp7a']);exit;
//                $idUser = $this->ion_auth->user()->row()->id;

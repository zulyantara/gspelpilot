...
class Pelatih_lanjutan extends Admin_Controller
{
    ...
    function check_nomykad()
    {
        ...
        // pertama-tama cek ke table markah_modul_status
    ...
    function tetapan_temuduga()
    {
        ...
        if ($btn_tetapkan_temuduga === "tetapkan temuduga")
        {
            ...
            $date = new DateTime($tarikh_waktu);
    ...
    {
        $select = "pkl.id,pbp.nama_penuh,pbp.no_mykad, p.no_pelatih, pkl.kategori_program, k.nama_kursus, k2.nama_kursus AS nama_kursus2, kls.nama_kluster, kls2.nama_kluster AS nama_kluster2,pkl.no_ssm, pkl.alamat_perniagaan, pp.nama_program AS nama_program_pertandingan, lpk.nama_program AS nama_program_khas, p.nama_bank, p.no_akaun, klsk.nama_kluster AS nama_kluster_kursus";
        $sql = "SELECT ".$select." FROM permohonan_butir_peribadi pbp LEFT JOIN temuduga_tetapan tt ON pbp.id=tt.id_permohonan AND tt.jenis_pelatih='LPP09' LEFT JOIN pelatih p ON pbp.id=p.id_permohonan AND p.jenis_pelatih='LPP04' LEFT JOIN settings_tawaran_kursus stk ON tt.id_kursus = stk.id LEFT JOIN ref_kursus k ON stk.id_kursus = k.id LEFT JOIN ref_kluster klsk ON k.id_kluster = klsk.id LEFT JOIN permohonan_kursus_lpp09 pkl ON pbp.id = pkl.id_permohonan LEFT JOIN ref_kluster kls ON pkl.id_kluster = kls.id LEFT JOIN settings_tawaran_kursus stk2 ON pkl.id_settings_tawaran_kursus = stk2.id LEFT JOIN ref_kursus k2 ON stk2.id_kursus = k2.id LEFT JOIN ref_kluster kls2 ON k2.id_kluster = kls2.id LEFT JOIN ref_lpp09_program_pertandingan pp ON pkl.id_program_pertandingan = pp.id LEFT JOIN ref_lpp09_program_khas lpk ON pkl.id_program_khas = lpk.id WHERE pbp.id=".$id." and pkl.id=".$id_pk;
        // echo $sql;exit;
        ...
        if ($row->kategori_program == "LL")
        {
            ...
            $this->mViewData["nama_kluster"] = $row->nama_kluster_kursus == NULL ? $row->nama_kluster2 : $row->nama_kluster;
        }
    ...
    function cetakan_surat_tawaran()
    {
        ...
        $res_kursus = $this->plm->get_v_giatmara_kursus($wk, "result", "distinct", "id_setting_penawaran_kursus, nama_kursus");
        ...
        if ($this->input->post('btn_tetapkan') === "tetapkan")
        {
            ...
            $where["id_settings_tawaran_kursus"] = $filter_kursus;
...
EOT;
	...
	function generate_pdf($id_permohonan, $stream = true, $template = null)
	{
		...
		$data["kursus_lpp09"] = $this->pm->get_kursus_lpp09($id_permohonan);
    ...
    function pendaftaran()
    {
        ...
        if ($this->input->post('btn_tetapkan') === "tetapkan")
        {
            ...
            $where["id_settings_tawaran_kursus"] = $this->input->post('filter_kursus');

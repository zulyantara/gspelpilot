...
class Auth extends CI_Controller {
	...
	public function login(){
		...
		//validate form input
	//	$this->form_validation->set_rules('username', 'Username', 'callbackend_username_check');
		...
		if ($this->form_validation->run() == true){
			...
			if ($this->ion_auth->login($this->input->post('username'), $this->input->post('password'), $remember)){
				...
				//redirect them backend to the home page
				$this->session->set_flashdata('message', $this->lang->line('login_successful'));
				redirect('admin/dashboard', 'refresh');
			}
			else{
				// if the login was un-successful
				// redirect them backend to the login page
	...
	public function user(){
		...
		elseif (!$this->ion_auth->is_superadmin()){
			...
			$this->_render_page('backend/auth/user', $this->data);
		}
	}
	...
	public function create_user(){
		...
		if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data)){
			// check to see if we are creating the user | redirect them backend to the admin page
			$this->session->set_flashdata('message', $this->ion_auth->messages());
	...
	public function edit_user($id){
		...
		if (isset($_POST) && !empty($_POST)){
			...
			if ($this->form_validation->run() === TRUE){
				...
				if($this->ion_auth->update($user->id, $data)){
					// mengarahkan kembali ke halaman admin apabila bukan superadmin
					$this->session->set_flashdata('message', $this->ion_auth->messages() );
		...
		$this->_render_page('backend/auth/edit_user', $this->data);
	}
	...
	public function users_group(){
		...
		elseif (!$this->ion_auth->is_superadmin()){
			redirect('admin/dashboard', 'refresh');
				...
				$this->_render_page('backend/auth/users_group_list', $this->data);
			}
	}
	...
	public function create_group(){
		...
		elseif (!$this->ion_auth->is_superadmin()){
			...
			$this->load->view('backend/auth/create_group', $this->data);
		}
	}
	...
	public function edit_group($id_group){
		...
		if ($row) {
			...
			$this->load->view('backend/auth/users_group_form', $data);
		}
		else {
	...
	public function reset_password($code = NULL){
		...
		{
			// if the code is invalid then send them backend to the forgot password page
			$this->session->set_flashdata('message', $this->ion_auth->errors());
			redirect("auth/forgot_password", 'refresh');
		}
	}
